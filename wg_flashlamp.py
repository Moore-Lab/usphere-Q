...
Module for WG3: GW Instek AFG-2225 waveform generator controlling the
flash lamp.

  CH1 — pulse trigger to flash lamp           (controls flash rate)
  CH2 — DC bias to lamp electrodes            (controls photons per flash)
...
DEVICE_NAME = "Flash Lamp (INSTEK AFG-2225 — WG3)"
...
class FlashLampController:
    """
    Manages WG3 (INSTEK AFG-2225) for flash lamp control.

    Typical usage
    -------------
    ctrl = FlashLampController(config)
    ctrl.connect()          # called when user clicks Connect in GUI
    ctrl.enable()           # called by control loop — starts flashing
    ctrl.disable()          # called by control loop — stops flashing
    ...
    def _apply_waveform(self) -> None:
        """Program CH1 (pulse trigger) and CH2 (DC bias) on the instrument."""
        rate  = self._parse("flash_rate_hz")
        amp   = self._parse("pulse_amplitude_v")
        width = self._parse("pulse_width_s")
        bias  = self._parse("dc_bias_v")

        self._afg.setup_pulse(1, frequency=rate, amplitude=amp, offset=0.0)
        if rate > 0:
            self._afg.waveform.set_pulse_width(1, width)
        # CH2: DC bias via sine with minimum amplitude
        self._afg.setup_sine(2, frequency=1.0, amplitude=0.001, offset=bias)
    ...
    def enable(self) -> bool:
        """Apply waveform config and turn on both outputs."""
        if not self.is_connected:
            raise RuntimeError("Flash lamp not connected")
        self._apply_waveform()
        ok1 = self._afg.output_on(1)
        ok2 = self._afg.output_on(2)
        return ok1 and ok2
    ...
    def disable(self) -> bool:
        ...
        if not self.is_connected:
            raise RuntimeError("Flash lamp not connected")
        ok1 = self._afg.output_off(1)
        ok2 = self._afg.output_off(2)
        return ok1 and ok2

    # ------------------------------------------------------------------
    # Direct parameter control (for experiment scripts)
    # ------------------------------------------------------------------

    def set_flash_rate(self, rate_hz: float) -> bool:
        """Change CH1 pulse frequency without touching other params."""
        if not self.is_connected:
            return False
        return self._afg.set_frequency(1, rate_hz)

    def set_electrode_voltage(self, voltage_v: float) -> bool:
        """Change CH2 DC offset (lamp electrode bias)."""
        if not self.is_connected:
            return False
        return self._afg.set_offset(2, voltage_v)

    def get_flash_rate(self) -> float | None:
        """Read current CH1 frequency."""
        if self._afg:
            return self._afg.get_frequency(1)
        return None

    def get_electrode_voltage(self) -> float | None:
        """Read current CH2 offset."""
        if self._afg:
            return self._afg.get_offset(2)
        return None
